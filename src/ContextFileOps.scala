...
object ContextFileOps:
  ...
  def readContextEntries(contextEntriesFile: Path): IO[ContextEntries] = IO.pure {
    if (os.exists(contextEntriesFile))
      ...
      val contextEntries = decode[Json](contextEntriesJson).getOrElse(Json.obj()).as[Map[
        String,
        DawnCtl.ContextEntry
      ]].getOrElse(Map.empty)
  ...
  def storePublicKey(
    ...
    for
      ...
      updatedContextEntries = cef.copy(entries =
        cef.entries + (name -> ContextEntry(did = "", publicKey = Some(publicKey)))
      )
  ...
  def getPublicKey(contextEntriesFile: Path, name: String): IO[Option[String]] =
    for
      cef    <- readContextEntries(contextEntriesFile)
      b64key <- IO.pure(cef.entries.get(name).flatMap(_.publicKey))
