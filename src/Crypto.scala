...
object Crypto:
  ...
  def storePrivateKey(
    ...
    keyStore.setKeyEntry(s"$alias", keyPair.getPrivate(), password.toCharArray(), certificateChain)
  ...
  def getPrivateKey(keyStorePath: os.Path, alias: String, password: String): IO[Key] =
    for
